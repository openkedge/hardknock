...
Terminal plans are immutable and repeated `run` returns the stored result. Concurrent executors cannot run the same plan. Hard process death is not automatically resumable: a Running plan can retain partial engine references, but requires inspection and a new plan. Ordinary cancellation is tested; escaped process sessions, disk failure, and adversarial commands are not contained by Git worktrees.

V0.9 adds provider requirements and capability metadata, but the curriculum engines have not been migrated wholesale to container execution. A planner must reject a trial whose declared isolation/effect needs exceed its selected provider; it must not downgrade and continue. Existing bundled curricula remain explicit trusted local Git fixtures, and their historical evidence remains labeled cooperative. Container-backed autonomous curricula, full process/VM snapshots, native tool-call cost, remote packages, and continuous autonomous scheduling remain deferred.
