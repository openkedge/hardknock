...
## V0.6 optional development context

Set `[development] bridge_context = true` to include `development_context` in session-start/resume and context-request replies. The optional field is absent by default and is omitted if adding it would exceed `bridge.max_context_bytes`. It contains `relevant` item IDs, `known_unknowns`, `stale_items`, `contradictions`, bounded `recommendations`, and `auto_run: false`. There are no raw transcripts, full procedures, experiments, or automatic curriculum requests in the bundle.

The context is evidence, not system authority. A changed agent/model preserves original provenance and may suggest revalidation; it does not confer independent replication. Cache activation applies the shared freshness policy. Optional projection assembly occurs outside the session mutex; pre-tool decisions continue to use the in-memory cache. Scope, exact local user policy, capability limits and permission behavior remain unchanged.
