...
pub fn compare_snapshots(
    ...
    let predictive_change =
        |field: fn(&crate::predictive::ForecastQualitySummary) -> Option<f64>| {
            change(
                field(&a.predictive.forecast_quality),
                field(&b.predictive.forecast_quality),
                a.predictive.forecast_quality.resolved_forecasts as u64,
                b.predictive.forecast_quality.resolved_forecasts as u64,
            )
        };
    GrowthReport{from:a.id.clone(),to:b.id.clone(),comparisons,regressions,median_recovery_ms:change(a.metrics.median_time_to_recovery_ms.map(|v|v as f64),b.metrics.median_time_to_recovery_ms.map(|v|v as f64),a.metrics.recovery_latency_samples,b.metrics.recovery_latency_samples),hardened_skills:change(Some(a.metrics.hardened_skill_count as f64),Some(b.metrics.hardened_skill_count as f64),a.artifact_counts.skills,b.artifact_counts.skills),runtime_adaptation:RuntimeGrowth{experiments_per_task:change(a.runtime_control.experiments_per_task,b.runtime_control.experiments_per_task,runtime_samples(&a.runtime_control),runtime_samples(&b.runtime_control)),unnecessary_intervention_rate:change(a.runtime_control.unnecessary_intervention_rate,b.runtime_control.unnecessary_intervention_rate,a.runtime_control.avoided_failures+a.runtime_control.unnecessary_interventions,b.runtime_control.avoided_failures+b.runtime_control.unnecessary_interventions),recovery_success_rate:change(a.runtime_control.recovery_success_rate,b.runtime_control.recovery_success_rate,a.runtime_control.decisions.get(&crate::runtime::RuntimeDecisionKind::Recover).copied().unwrap_or(0),b.runtime_control.decisions.get(&crate::runtime::RuntimeDecisionKind::Recover).copied().unwrap_or(0))},predictive_resilience:PredictiveGrowth{validated_signatures:change(Some(a.predictive.validated_signatures as f64),Some(b.predictive.validated_signatures as f64),a.predictive.early_warning_signatures as u64,b.predictive.early_warning_signatures as u64),precision:predictive_change(|quality|quality.precision),recall:predictive_change(|quality|quality.recall),false_positive_rate:predictive_change(|quality|quality.false_positive_rate),median_warning_lead_actions:predictive_change(|quality|quality.median_warning_lead_actions),avoided_failure_rate:predictive_change(|quality|quality.avoided_failure_rate),unnecessary_intervention_rate:predictive_change(|quality|quality.unnecessary_preventive_intervention_rate),validated_preventive_interventions:change(Some(a.predictive.validated_preventive_interventions as f64),Some(b.predictive.validated_preventive_interventions as f64),a.predictive.validated_preventive_interventions as u64,b.predictive.validated_preventive_interventions as u64)},note:"Observed behavior and evidence, not general intelligence or causal proof. Task mixes may differ even in disjoint windows. Runtime adaptation and predictive resilience are based on recorded decisions, forecasts, and feedback; they are descriptive, not automatic policy learning.".into()}
}
